...
`utils.js` exports: `calculateTip`, `toCelsius`, `toFahrenheit`, plus the two additions `formatCurrency` and `toLetterGrade` (the lab's `getGrade`, renamed). All JSDoc-commented. `app.js` imports all five and exercises them (tip calculator, temperature conversions, currency formatting, letter grades 95 to A through 55 to F).
...
Connections: `response.json()` from the API lab is the same operation (text in, object out), async version. The mirror twin is `JSON.stringify(obj)` (object to text) for *sending* JSON. Parse to receive, stringify to send.
