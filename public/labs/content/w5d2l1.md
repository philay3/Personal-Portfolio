...
> **Side task (self-study):** event handling deep dive. How events flow from DOM to handler to state setter, synthetic events, `event.target`, and handler-as-prop patterns. Scheduled as independent practice.
...
- **Level 2 - `skills` badges:** the string-to-array-to-elements pipeline:
...
- count 3, color green, message "Keep going!"
- count 7, color orange, message "Getting warm..."
- count 10, color red, message "You made it to 10!"
...
`.some()` answers "does at least one item match?". On a duplicate, no setState is called, so no re-render happens. State unchanged = screen unchanged.
